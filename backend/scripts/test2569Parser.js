...
async function test2569() {
  ...
  console.log(`\n🎉 Decoded ${resultList.length} unique subjects in CS69 (2569)!`);
  console.log("\nSample 1:", resultList[0]);
  console.log("\nSample 2 (middle):", resultList[Math.floor(resultList.length / 2)]);
  console.log("\nSample 3 (AI):", resultList.find(s => s.subject_code === "040613711" || s.title_th.includes("ปัญญาประดิษฐ์")));
}

test2569().catch(console.error);

...
export default function ConsultDetail() {
  ...
  // 📋 ข้อมูลอาจารย์และนักศึกษา อ้างอิงตามรูปภาพที่ส่งมา
  const advisorInfo = [
    {
      id: 1,
  ...
  return (
    <div className="bg-white font-['Prompt'] min-h-screen flex flex-col text-left">
      ...
      <main className="max-w-6xl mx-auto w-full px-6 py-12 flex-grow space-y-12">
        ...
        <div className="space-y-20">
          {advisorInfo.map((advisor) => {
            const filteredStudents = advisor.students.filter(s => 
              ...
              <div key={advisor.id} className="grid grid-cols-1 md:grid-cols-4 gap-8 items-start">
                ...
                <div className="md:col-span-1 space-y-4">
                  ...
                  <div className="px-1 text-left">
                    <h3 className="text-sm font-bold text-[#3F51B5] leading-snug">{advisor.name}</h3>
                    <p className="text-[11px] text-slate-500 mt-2 flex items-center gap-1.5 break-all">
                      <Mail size={12} className="shrink-0" /> {advisor.email}
                    </p>
                  </div>
                </div>
                      ...
                      <tbody className="divide-y divide-slate-100">
                        {filteredStudents.map((student, index) => (
                          <tr key={student.id} className="hover:bg-slate-50 transition-colors">
                            ...
                            <td className="px-4 py-3 text-slate-700">{student.code}</td>
                            <td className="px-4 py-3 text-slate-600">{student.title}</td>
                            <td className="px-4 py-3 font-bold text-slate-800">{student.firstName}</td>
                            <td className="px-4 py-3 font-bold text-slate-800">{student.lastName}</td>
                            <td className="px-4 py-3 text-center">
                              <span className="text-[11px] font-black text-slate-400">{student.room}</span>
                            </td>
                          </tr>
                        ))}
                      </tbody>
